...
## 2025-03-02 - Symlink-based Zip Slip Vulnerability
**Vulnerability:** The zip extraction logic in `upload_project` used `os.path.abspath()` coupled with a simple `.startswith()` check to prevent Zip Slip attacks. However, `os.path.abspath()` does not resolve symlinks. A maliciously crafted zip containing a symlink pointing outside the extraction directory, followed by a file inside that symlink, would bypass the `.startswith()` check and result in arbitrary file write (Zip Slip).
**Learning:** `os.path.abspath()` is insufficient for path traversal prevention because it merely normalizes the string representation of a path without checking the actual filesystem or resolving symlinks. `os.path.realpath()` must be used to follow symlinks to their ultimate destination, and this must be applied to both the destination base folder AND the extracted file destination to properly match symlinks that could exist on systems like macOS (e.g. `/var` symlinked to `/private/var`).
**Prevention:** Always use `os.path.realpath()` when resolving paths from untrusted archives before extraction for both the temporary directory and the combined file path. Additionally, use `os.path.commonpath()` (wrapped in a `try/except ValueError` for cross-drive checks on Windows) instead of `.startswith()` to securely guarantee the resolved path remains strictly within the intended base directory.
