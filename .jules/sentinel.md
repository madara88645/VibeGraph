...
## 2025-04-15 - Unrestricted Hidden File Upload (Path Traversal variant)
**Vulnerability:** The `normalize_uploaded_filename` and zip extraction loops stripped `../` for directory traversal but allowed uploading hidden files (e.g. `.env`) and hidden directories (e.g. `.git/config`) into the temp directory. While snippet read endpoints correctly blocked reading these files, attackers could potentially use uploads to stage arbitrary malicious payload files disguised as hidden files on the disk.
**Learning:** `os.path.commonpath` alone does not validate the type or naming restrictions of files, meaning `foo/.env` evaluates as safely inside `foo/`. Furthermore, standard backend defenses on read don't automatically stop those files from being created by write endpoints.
**Prevention:** Explicitly inspect both direct file uploads and ZIP members via `split("/")` and block paths containing any hidden segment (e.g., `part.startswith(".")`) *during* the upload extraction phase, preventing them from being written to disk at all.
