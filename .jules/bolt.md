...
## 2026-05-29 - Optimize O(N^2) list membership check with O(1) set tracking
**Learning:** In Python performance optimization, replacing a list comprehension containing an O(1) set membership check (e.g., `[k for k in dict if k not in visited_set]`) with a dynamically updated `unvisited_set` eliminates the O(N) array allocation overhead from executing repeatedly within a fallback selection loop (resulting in O(N^2) complexity).
**Action:** To eliminate O(N^2) time complexity when repeatedly filtering unvisited items inside a loop (e.g., during graph traversal fallbacks), initialize an `unvisited_set` before the loop and use `.discard(item)` as items are visited, rather than evaluating an O(N) list comprehension like `[n for n in items if n not in visited]` on every iteration.
