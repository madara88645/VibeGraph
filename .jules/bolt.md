...
## YYYY-MM-DD - Safe dictionary unpacking optimization
**Learning:** When optimizing dictionary unpacking and list comprehensions in Python, avoid replacing them with in-place mutations (e.g., `item['key'] = val`) without verifying reference safety, as this can introduce critical regressions by mutating shared state. To optimize safely while preserving immutability, conditionally bypass the shallow copy if the value is already correct (e.g., `[{**item, 'key': val} if item.get('key') != val else item for ...]`).
**Action:** When I encounter dictionary comprehension that unpacks and modifies a key like `{**step, 'step': index}`, I will replace it with `[{**step, 'step': index} if step.get('step') != index else step for index, step in enumerate(...)]` to avoid unnecessary object allocation overhead.
