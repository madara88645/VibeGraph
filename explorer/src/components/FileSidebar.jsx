    ...
    >
        ...
        >
          <div className="sidebar-content">
            <button
              ...
              aria-label={`All Files, ${totalNodeCount || 0} nodes`}
            >
            ...
            {files.map((file) => {
              ...
              // Build a rich aria-label
              let ariaLabel = `${file}, ${stats.count || 0} nodes`;
              if (isSelected && stats.types) {
                const typeNames = {
                  function: "functions",
                  class: "classes",
                ...
                const typeDescriptions = Object.entries(stats.types).map(
                  ([type, count]) => `${count} ${typeNames[type] || type}`,
                );
                if (typeDescriptions.length > 0) {
                  ariaLabel += `. Contains ${typeDescriptions.join(", ")}`;
                }
              }
              ...
              return (
                ...
                >
                  ...
                  {dirName && (
                    <div className="file-dir" title={dirName}>
                      {dirName}
                    </div>
                  )}
      ...
      {activeTab === "deps" && (
        ...
        >
          ...
          {deps &&
            Object.entries(deps).map(([file, info]) => {
              const shortName = getShortName(file) || file;
              const isSelected = file === selectedFile;
              ...
              return (
                <div
                  key={file}
                  className={`deps-file ${isSelected ? "selected" : ""}`}
                >
                  <button
                    ...
                    aria-label={file}
                  >
                    <span className="deps-file-icon" aria-hidden="true">
                      📄
                    </span>
                    <span className="deps-file-name" title={file}>
                      {shortName}
                    </span>
                  </button>
                      ...
                      {imports.map((imp, i) => {
                        const impName =
                          typeof imp === "string"
                            ...
                            : imp.module || imp.name;
                        const details =
                          typeof imp === "object" ? imp.names : null;
                        return (
