...
def create_app() -> FastAPI:
    ...
    allowed_domains_env = os.getenv("VIBEGRAPH_ALLOWED_CORS_DOMAINS")
    allowed_domains = []
    ...
    for origin in origins_list:
        if origin == "*":
            if allowed_domains:
                logger.warning(
                    "Wildcard CORS origin '*' is blocked when VIBEGRAPH_ALLOWED_CORS_DOMAINS is strictly enforced."
                )
                continue
            ...
            if allowed_domains:
                ...
                if parsed.hostname not in allowed_domains:
                    logger.warning(
                        f"Blocked CORS origin due to domain mismatch: {origin}"
                    )
                    continue
            validated_origins.append(origin)
        else:
    ...
    async def validation_exception_handler(
        ...
        """Prevent input reflection in validation errors."""
        errors = exc.errors()
        safe_errors = []
        for error in errors:
            safe_errors.append(
