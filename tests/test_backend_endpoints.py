...
class TestAnalyzerForSearch(unittest.TestCase):
    ...
    def test_analyze_single_file_method_syntax_error_no_merge(self):
        """Invalid syntax in _analyze_single_file with merge=False."""
        analyzer = CodeAnalyzer()
        broken_file = os.path.join(self.proj.tmpdir, "broken.py")
        with open(broken_file, "w", encoding="utf-8") as f:
            f.write("def broken(:\n    pass\n")

        result = analyzer._analyze_single_file(broken_file, merge=False)
        self.assertIn("error", result)
        self.assertTrue(result["error"].startswith("Syntax error in"))

    def test_analyze_single_file_method_syntax_error_merge(self):
        """Invalid syntax in _analyze_single_file with merge=True."""
        analyzer = CodeAnalyzer()
        broken_file = os.path.join(self.proj.tmpdir, "broken.py")
        with open(broken_file, "w", encoding="utf-8") as f:
            f.write("def broken(:\n    pass\n")

        result = analyzer._analyze_single_file(broken_file, merge=True)
...
# ---------------------------------------------------------------------------
# Input length validation (DoS protection) tests
# ---------------------------------------------------------------------------
...
class TestInputLengthValidation(unittest.TestCase):
    """Verify that over-limit payloads are rejected with HTTP 422."""
    ...
    def test_explain_node_id_too_long_returns_422(self):
        resp = self.client.post(
            "/api/explain",
            json={"node_id": "x" * (MAX_NODE_ID_LENGTH + 1), "level": "beginner"},
        )
        self.assertEqual(resp.status_code, 422)

    def test_explain_file_path_too_long_returns_422(self):
        resp = self.client.post(
            "/api/explain",
            json={"file_path": "a" * (MAX_FILE_PATH_LENGTH + 1), "node_id": "main"},
        )
        self.assertEqual(resp.status_code, 422)

    # /api/snippet ---------------------------------------------------------

    def test_snippet_node_id_too_long_returns_422(self):
        resp = self.client.post(
            "/api/snippet",
            json={"node_id": "x" * (MAX_NODE_ID_LENGTH + 1)},
        )
        self.assertEqual(resp.status_code, 422)

    def test_snippet_file_path_too_long_returns_422(self):
        resp = self.client.post(
            "/api/snippet",
            json={"file_path": "a" * (MAX_FILE_PATH_LENGTH + 1), "node_id": "main"},
        )
    ...
    def test_chat_question_too_long_returns_422(self):
        resp = self.client.post(
            "/api/chat",
            json={"question": "q" * (MAX_QUESTION_LENGTH + 1), "history": []},
        )
        self.assertEqual(resp.status_code, 422)

    def test_chat_history_too_many_items_returns_422(self):
        history = [
        ...
        resp = self.client.post(
            "/api/chat",
            json={"question": "What does this do?", "history": history},
        )
        self.assertEqual(resp.status_code, 422)

    def test_chat_message_content_too_long_returns_422(self):
        resp = self.client.post(
            "/api/chat",
            json={
                "question": "What does this do?",
                "history": [{"role": "user", "content": "x" * (MAX_CONTENT_LENGTH + 1)}],
            },
        )
        self.assertEqual(resp.status_code, 422)
    ...
    def test_learning_path_file_path_too_long_returns_422(self):
        resp = self.client.post(
            "/api/learning-path",
            json={"file_path": "a" * (MAX_FILE_PATH_LENGTH + 1)},
        )
        self.assertEqual(resp.status_code, 422)
